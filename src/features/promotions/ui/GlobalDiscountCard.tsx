    ...
    startTransition(async () => {
      ...
      setMessage(
        result.value.percentage == null
          ? pageCopy.globalCleared
          : pageCopy.globalSet.replace(
              "{value}",
  ...
  return (
    <article className="flex h-full flex-col rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
      <div className="mb-4 flex items-start gap-3">
        ...
        <div>
          <h2 className="text-base font-semibold text-gray-900">
            {pageCopy.globalTitle}
          </h2>
          <p className="text-sm text-gray-500">{pageCopy.globalSubtitle}</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Button
          ...
          onClick={() => {
            ...
            if (parsed === "invalid") {
              setError(pageCopy.globalInvalid);
              return;
            }
            save(parsed);
          }}
      ...
      <p className="mt-3 text-sm text-gray-500">
        {saved == null
          ? pageCopy.globalNone
          : pageCopy.globalActive.replace("{value}", String(saved))}
      </p>
