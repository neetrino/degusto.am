...
/** SQL AND-clause matching `buildComboProductWhereBase` (products alias `p`). */
function buildComboProductBaseFilterSql(
...
export async function fetchComboMenuCategoryProductCounts(
  ...
  const baseFilter = buildComboProductBaseFilterSql(locale, query);
