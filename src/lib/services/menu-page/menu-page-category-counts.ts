...
import type {
  MenuCategoryProductCounts,
  MenuPageComboScope,
  MenuPageFilterQuery,
} from './menu-page-query.types';

export function toMenuCountNumber(value: unknown): number {
...
function buildComboCategoryFilterSql(comboScope: MenuPageComboScope): Prisma.Sql {
  if (comboScope === 'only') {
    return Prisma.sql`
      EXISTS (
        SELECT 1
...
/** SQL AND-clause matching `buildMenuProductWhereBase` (products alias `p`). */
function buildMenuProductBaseFilterSql(
  ...
  const clauses: Prisma.Sql[] = [
    ...
    buildComboCategoryFilterSql(comboScope),
  ];
 ...
 */
export async function fetchMenuCategoryProductCounts(
  locale: StorefrontLocale,
  query: MenuPageFilterQuery,
  comboScope: MenuPageComboScope,
  slugsToCount: string[]
): Promise<MenuCategoryProductCounts> {
  const baseFilter = buildMenuProductBaseFilterSql(locale, query, comboScope);
  ...
  for (const row of slugCountRows) {
    if (typeof row.slug === 'string' && row.slug) {
      countBySlug[row.slug] = toMenuCountNumber(row.total);
    }
  }

  return {
    allProductCount: toMenuCountNumber(allCountRows[0]?.total),
    countBySlug,
  };
}
