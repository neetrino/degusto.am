...
import {
  SHOP_ABOVE_FOLD_DESKTOP_IMAGE_COUNT,
  SHOP_ABOVE_FOLD_MOBILE_IMAGE_COUNT,
...
function StaticMenuProductCard({
  card,
  imagePriority = false,
}: {
  card: MenuCard;
...
export function StorefrontMenuPageShell({
  ...
  const shouldRenderMobileProductsList = showMobileProductsList;
            ...
            {cards.length > 0 ? (
              <div className={`mt-8 ${MOBILE_SHOP_PRODUCTS_GRID_CLASS}`}>
                {cards.map((card, index) => (
                  <StaticMenuProductCard
                    key={card.id}
                    card={card}
                    imagePriority={!renderDesktopLayout && index < SHOP_ABOVE_FOLD_MOBILE_IMAGE_COUNT}
                  />
                ))}
              </div>
            ) : (
