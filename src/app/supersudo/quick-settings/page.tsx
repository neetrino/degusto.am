...
export default function QuickSettingsPage() {
  ...
  const fetchSettings = useCallback(async () => {
    try {
      ...
      const settings = await adminGet<AdminSettingsResponse>('/api/v1/admin/settings');
  ...
  const fetchProducts = useCallback(async (page: number, search: string) => {
    try {
      ...
      const response = await adminGet<AdminProductsResponse>('/api/v1/admin/products', { params });
  ...
  const fetchCategories = useCallback(async () => {
    try {
      ...
      const response = await adminGet<{ data: AdminCategory[] }>('/api/v1/admin/categories');
      if (response?.data && Array.isArray(response.data)) {
