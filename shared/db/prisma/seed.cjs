...
/** Per-variant-index sauce/garlic so each of the 4 SKUs has a unique full preference tuple. */
  ...
  for (let i = 0; i < titles.length; i++) {
    ...
    const product = await prisma.product.create({
      data: {
        ...
        primaryCategoryId,
        attributeIds: [spicyAttributeId, greensAttributeId, sauceAttributeId, garlicAttributeId],
